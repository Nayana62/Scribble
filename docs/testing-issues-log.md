...
- **Resolution**: [PHASE-N] PR/commit — what was changed.
...
- **Resolution**: [PHASE-4] Server `playAgain` converted to ack-based. Added host check (`NOT_HOST`) and state check (`INVALID_STATE`). Client wraps emit with an ack callback + 5-second timeout, showing an inline error on the end screen for any failure case.
- **Status**: Fixed
...
- **Resolution**: [PHASE-2] + [PHASE-4] Immediate `rooms.delete()` replaced with a 60-second grace timer (`gracePeriod.js`). When the grace period elapses a `roomClosed` event (with `reason: "empty"`) is broadcast to any sockets still in the Socket.IO room. Client listens for `roomClosed`, shows a notice, and dispatches `ROOM_CLOSED` → home screen.
- **Status**: Fixed
